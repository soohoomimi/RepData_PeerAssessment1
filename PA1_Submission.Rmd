...
I averaged steps for each interval, averaging across all days (e.g. the output from daily activity pattern, part 1).
...
These values differ slightly from the estimates in the first part of the assignment. The mean of total steps is not different after NA's are imputed with interval averages, but what does shift is the median, which now matches the mean exactly. Because we are calculating sums, any imputing of positive values in place of NAs will increase the average total and median total. 
